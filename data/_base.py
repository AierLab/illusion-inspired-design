...
class MyDataset(Dataset):
    def __init__(self, data_dir, transforms=None, strength=None):
        ...
        self.data_info = get_img_info(data_dir, strength)
...
def get_img_info(data_dir, strength):
    # Load CSV
    ...
    if strength is not None:
        ...
        lower_bound = strength - 0.05
        upper_bound = strength + 0.05
        df = df[
            (df['Illusion_Strength_Std'] >= lower_bound) &
            (df['Illusion_Strength_Std'] <= upper_bound)
