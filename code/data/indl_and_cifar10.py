import torch
from data_cifar10 import train_dataset, test_dataset
...
class LabelModifier(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = torch.where(label == 0, torch.tensor(10), label)
        label = torch.where(label == 1, torch.tensor(11), label)
...
trainloader_combined = DataLoader(combined_trainset_final, batch_size=64, shuffle=True)

# Create DataLoader for combined testing dataset (with labels changed to 11 and 12)
testloader_combined = DataLoader(combined_testset_final, batch_size=64, shuffle=False)
