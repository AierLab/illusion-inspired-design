...
class CompositionModel(LightningModule):
    def __init__(self, steps_per_epoch, ckpt_path_A, ckpt_path_B, lr=1e-3):
        ...
        self.model_A = ResNet50.load_from_checkpoint(ckpt_path_B, map_location="cpu").model
        ...
        self.model_B = ResNet50.load_from_checkpoint(ckpt_path_A, map_location="cpu").model
...
wandb_logger = WandbLogger(project="illusion_augmented_models", name="composition_reverse", log_model=True)

# Experiment name and setup
exp_name = "image_classification_model_augmented_illusion_model"

# Callbacks
checkpoint_callback = ModelCheckpoint(
    monitor="val_acc",
    dirpath="./models/comp_reverse/",
    filename="comp_reverse_{epoch:02d}-{val_acc:.2f}",
    save_top_k=1,
    mode="max",
)
...
checkpoint_dir = "./models/comp_reverse/"
...
model = CompositionModel(len(trainloader_indl), ckpt_path_A, ckpt_path_B)
...
trainer = Trainer(
    ...
    accelerator="auto",
    devices=2  # Adjust this according to the number of devices (e.g., GPUs)
)
